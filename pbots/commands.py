...
def _wrap_command(source: Source, logger: Logger, func: Callable[[Source, Logger], None]) -> None:
    """
    ...
    :param func: function that implements the command (it must take the source and the logger as arguments)
    """
    try:
        func(source, logger)
...
def run_command(source: Source) -> None:
    """
    Run a PBOTS command (usually a function that scrapes and mails) on a separate thread.
    :param source: scraping data source
    """
    logger = getLogger("commands.{}".format(source.command))
    logger.info("run_command: {}".format(source.command))
    try:
        this_module = sys.modules[__name__]
        func = getattr(this_module, source.command)
        t = threading.Thread(target=_wrap_command, args=(source, logger, func), kwargs={})
        t.setDaemon(True)
        t.start()
...
def _add_publications(source: Source, logger: Logger, json_as_text: str) -> None:
    """
    Add the publications found by a scraper to the DB.
    :param source: scraping data source
    :param logger: source-specific logger
    :param json_as_text: the JSON (as string) returned by the scraping script
    """
...
def send_email(recipients: List[str], title: str, publications: Union[List[Publication], List[Dict]]) -> None:
    """
    ...
    email.mixed_subtype = 'related'  # This is required for images to be displayed within the message body
    ...
    # The logo will be added as MIME image attachment and referenced in the img src via cid:xxxx because:
    #   1. Linking an external image triggers email client warnings
    #   2. Base64 encoded src is not supported by Gmail
    logo_file = os.path.join(os.path.dirname(__file__), "..", "lilium", "static", "img", "pbse-191x32.png")
...
def _run_mailing_list(source: Source,
                      logger: Logger,
                      publication_transformer: Optional[Callable[[Publication], Publication]] = None) -> None:
    """
    Send newly found publications to the mailing list members.
    :param source: scraping data source
    :param logger: source-specific logger
    :param publication_transformer: optional function to modify the publications that will be sent
    """
    ...
    if source.last_id is None:
        ...
        publications = Publication.objects.filter(source=source, id__gt=source.last_id)
    logger.info("Publications found: {}".format(len(publications)))
    if len(publications) == 0:
        logger.info("Nothing to send.")
        return
    # Create the email content
    template = loader.get_template("mailing_list/publications.html")
    if publication_transformer:
        publications = [publication_transformer(p) for p in publications]
    ...
    logger.info("Email sent; the last publication sent for source {} has ID {}".format(source.id, source.last_id))


def _run_scraper_phantomjs(script_file: str, arguments: List[str], source: Source, logger: Logger) -> None:
    """
    Run a PhantomJS-based scraper.
    :param script_file scraper source code (JavaScript) file name (it must be located under pbots/tools)
    :param arguments array of extra arguments to be passed to the script, if needed
    :param source: scraping data source
    :param logger: source-specific logger
    """
    tools_dir = os.path.join(os.path.dirname(__file__), "tools")
    script_file = os.path.join(tools_dir, script_file)
    output = subprocess.check_output([settings.PHANTOMJS_EXE, script_file] + arguments)
    _add_publications(source, logger, output)


def _run_scraper_casperjs(script_file: str, arguments: List[str], source: Source, logger: Logger) -> None:
    """
    ...
    tools_dir = os.path.join(os.path.dirname(__file__), "tools")
    script_file = os.path.join(tools_dir, script_file)
    output = subprocess.check_output([settings.CASPERJS_EXE, script_file] + arguments)
    _add_publications(source, logger, output)


def _run_scraper_python(script_file: str, arguments: List[str], source: Source, logger: Logger) -> None:
    """
    Run a Python-based scraper.
    :param script_file scraper source code (Python) file name (it must be located under pbots/tools)
    :param arguments array of extra arguments to be passed to the script, if needed
    :param source: scraping data source
    :param logger: source-specific logger
    """
...
def bollettino_regione_umbria_generale(source: Source, logger: Logger) -> None:
    ...
    :param logger: source-specific logger
    """
    _run_scraper_phantomjs("bollettino-regione-umbria.js", ["1"], source, logger)
    _run_mailing_list(source, logger, _bollettino_regione_umbria_publication_filter)
...
def bollettino_regione_umbria_avvisi(source: Source, logger: Logger) -> None:
    ...
    :param logger: source-specific logger
    """
    _run_scraper_phantomjs("bollettino-regione-umbria.js", ["2"], source, logger)
    _run_mailing_list(source, logger, _bollettino_regione_umbria_publication_filter)
...
def bollettino_regione_umbria_informazioni(source: Source, logger: Logger) -> None:
    ...
    :param logger: source-specific logger
    """
    _run_scraper_phantomjs("bollettino-regione-umbria.js", ["3"], source, logger)
    _run_mailing_list(source, logger, _bollettino_regione_umbria_publication_filter)
...
def albopretorio_comune_acquasparta(source: Source, logger: Logger) -> None:
    ...
    :param logger: source-specific logger
    """
    _run_scraper_casperjs("albopretorio-comune-acquasparta.js", [], source, logger)
...
def albopretorio_comune_montecastrilli(source: Source, logger: Logger) -> None:
    ...
    :param logger: source-specific logger
    """
    _run_scraper_python("albopretorio-comune-montecastrilli.py", [], source, logger)
    _run_mailing_list(source, logger)
...
def matrimoni_comune_montecastrilli(source: Source, logger: Logger) -> None:
    ...
    :param logger: source-specific logger
    """
    _run_scraper_python("matrimoni-comune-montecastrilli.py", [], source, logger)
    _run_mailing_list(source, logger)
